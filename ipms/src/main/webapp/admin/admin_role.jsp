...
<head>
    ...
    <link rel="stylesheet"
          href="<%=request.getContextPath()%>/frame/uikit/css/components/accordion.almost-flat.min.css">
    ...
    <style>
        ...
        .uk-panel-title, .uk-accordion-title {
            font-size: 14px;
        }
        ...
        .uk-grid, .uk-width-2-10, .uk-width-8-10, .uk-width-1-1, .uk-width-1-2 {
            margin: 0;
            padding: 0;
        }
    ...
    <div class="uk-width-8-10">
        ...
        <div class="uk-panel uk-panel-box">
            <ul class="uk-breadcrumb">
                ...
                <li>系统管理</li>
                <li class="uk-active">角色管理</li>
            </ul>
            <hr>
            <ul class="uk-subnav uk-subnav-line">
                <li>
                    <a href="<%=request.getContextPath()%>/admin/admin_role_add.jsp"
                       class="uk-button uk-button-success">新增</a>
                </li>
            </ul>
                    ...
                    <c:forEach items="${list }" var="t" varStatus="d">
                        <tr>
                            ...
                            <td class="uk-text-center">
                                <button class="uk-button uk-button-small" onclick="javascript:edit('${t.id}')">编辑
                                </button>
                                <button class="uk-button uk-button-small uk-button-danger"
                                        onclick="javascript:del('${t.id }')">删除
                                </button>
                                <button class="uk-button uk-button-small uk-button-primary"
                                        onclick="javascript:per('${t.id }')">权限
                                </button>
                            </td>
                        </tr>
                    </c:forEach>
...
<script>
    function del(id) {
        if (confirm("确定删除此记录吗？")) {
            $.get("<%=request.getContextPath()%>/role/delRole.do?id=" + id, function (data) {
                if (0 == data.result) {
    ...
    function edit(id) {
        window.location.href = "<%=request.getContextPath()%>/role/viewRole.do?id=" + id;
    }

    function per(id) {
        window.location.href = "<%=request.getContextPath()%>/role/getPerms.do?id=" + id;
    }
</script>
</html>