			...
			<div class="uk-grid">
				...
				<div class="uk-width-2-3">
					<div class="uk-form-row">
						<label class="uk-form-label" for="">栏目</label> 
					...
					<div class="uk-form-row">
						...
						<div class="uk-form-controls">
							 <input type="text" size="64" name="title"  value="${content.title }" placeholder="文章标题" required>
						</div>
					</div>
					<%-- <div class="uk-form-row">
								<label class="uk-form-label" for="">参加日期（选填）</label>
								<div class="uk-form-controls uk-form-controls-text">
									<input type="text" name="meettime" value="${content.meettime }"  readonly onClick="WdatePicker()" >&nbsp;&nbsp;&nbsp;
								</div>	
					</div> --%>
					<div class="uk-form-row">
						<label class="uk-form-label" >摘要</label>
						<div class="uk-form-controls">
							<textarea rows="3" cols="67" name="abstracts" id="abstracts" placeholder="选填,如果不填写会默认捉取正文前54个字" style="resize: none">${content.abstracts }</textarea>
						</div>
					</div>
					<div class="uk-form-row">
						...
						<!-- UEditor -->
						<div class="uk-form-controls">
							<script id="contents" type="text/plain" name="contents">${content.contents }</script>
							<!-- 配置文件 -->
					...
					<div class="uk-form-row">
						<c:if test="${content.status=='N'}">
							<button class="uk-button uk-button-success" type="submit">保存</button>
						</c:if>
						<c:if test="${content.status=='Y'}">
							<c:if test="${sysuser.type=='admin'}">
								<button class="uk-button uk-button-success" type="submit">保存</button>
							</c:if>
							<c:if test="${sysuser.type!='admin'}">
								<button class="uk-button uk-button-success" disabled>已审核，不能编辑</button>
							</c:if>
						</c:if>
						<button type="button" class="uk-button uk-button-danger" onclick="javascript:show('${content.id}')">预览</button>
					</div>
				</div>
			</div>
		</form>
...
<script type="text/javascript">
	$.ajax({
		...
		success : function(data) {
			...
			$("#topic>option[value=${content.topic}]").attr("selected",true);
		},
	...
	function show(id) {
		   window.open("<%=request.getContextPath()%>/cms_content/showContent.do?id=" + id, 'newwindow', 'height=627, width=375, top=150, left=380, toolbar=no, menubar=no, scrollbars=1, resizable=no, location=no, status=no') 
		}
	...
	$(function(){
		$('.myform').validate({
			...
			submitHandler:function(){
				...
				document.myform.action = "<%=request.getContextPath()%>/cms_content/updatContent.do";
